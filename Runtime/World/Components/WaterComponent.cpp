...
namespace Genome
{
    ...
    void WaterComponent::GenerateAsync()
    {
        if (m_is_generating)
        {
            LOG_WARNING("WaterComponent is already being generated, please wait...");
            return;
        }

        if (!m_height_map)
        {
            LOG_WARNING("You need to assign a height map before trying to generate a WaterComponent.");
        ...
        m_context->GetSubsystem<Threading>()->AddTask([this]()
            {
                ...
                if (GeneratePositions(positions, height_map_data))
                {
                    ...
                    m_progress_desc = "Generating WaterComponent vertices and indices...";
    ...
    bool WaterComponent::GeneratePositions(std::vector<Vector3>& positions, const std::vector<std::byte>& height_map)
    {
        if (height_map.empty())
        {
    ...
    bool WaterComponent::GenerateVerticesIndices(const std::vector<Vector3>& positions, std::vector<uint32_t>& indices, std::vector<RHI_Vertex_PosTexNorTan>& vertices)
    {
        if (positions.empty())
        {
    ...
    bool WaterComponent::GenerateNormalTangents(const std::vector<uint32_t>& indices, std::vector<RHI_Vertex_PosTexNorTan>& vertices)
    {
        if (indices.empty())
        {
    ...
    void WaterComponent::UpdateFromModel(const std::shared_ptr<Model>& model) const
    {
        if (!model)
        {
    ...
    void WaterComponent::UpdateFromVertices(const std::vector<uint32_t>& indices, std::vector<RHI_Vertex_PosTexNorTan>& vertices)
    {
        ...
        if (!m_model)
        {
            ...
            ResourceCache* resource_cache = m_context->GetSubsystem<ResourceCache>();
            m_model->SetResourceFilePath(resource_cache->GetProjectDirectory() + m_entity->GetName() + "_WaterComponent_" + std::to_string(m_id) + std::string(EXTENSION_MODEL));
